...
void forward(Cnn *cnn, CnnMemory *memory, double**image) {
    conv_forward(cnn->conv1, memory->conv1_buffer, image, IMG_H, IMG_W, relu); // out num_filtersx26x26
    pool(cnn->pool1, memory->pool1_buffer, memory->conv1_buffer);
    flatten(memory->pool1_buffer, memory->flat_buffer, NUM_FILTERS, cnn->pool1->in_h/2, cnn->pool1->in_w/2);
    ...
    dense_forw(cnn->dense3, memory->dense3_buffer, memory->dense2_buffer, none); // These are logits
    softmax(memory->dense3_buffer, memory->probs_buffer, NUM_CLASSES); // probability distribution

}

double backward(Cnn *cnn, CnnMemory *memory, double**image, unsigned char label) {

    double loss = softmax_cross_entropy_back(memory->probs_buffer, label, memory->grad_logits, NUM_CLASSES);

    dense_back_no_activation(cnn->dense3, memory->dense2_buffer, memory->grad_logits, memory->grad_dense2);
    dense_back_relu(cnn->dense2, memory->dense2_buffer, memory->dense1_buffer, memory->grad_dense2, memory->grad_dense1);
    dense_back_relu(cnn->dense1, memory->dense1_buffer, memory->flat_buffer, memory->grad_dense1, memory->grad_flat);

    unflatten(memory->grad_flat,
        memory->grad_pool,
    ...
    pool_back(cnn->pool1, memory->conv1_buffer, memory->pool1_buffer, memory->grad_pool, memory->grad_conv);
    conv_back(cnn->conv1, image, memory->conv1_buffer, memory->grad_conv, memory->grad_input);
...
int main() {
    // This blog has been of help with conceptual understanding of each step: https://victorzhou.com/blog/intro-to-cnns-part-2/
    // Statquest youtube channel has also been of great help with understanding backpropagation and its implementation
